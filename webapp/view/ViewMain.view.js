sap.ui.jsview("estadocuenta.EstadoCuenta.view.ViewMain", {
	...
	createContent: function (oController) {
		...
		var oFiltroCliente = new sap.m.Input(CONSTESTCUENTA.ID.FILTRO_CLIENTE,{
			change: [oController.filtrarCliente, oController],
			showValueHelp: true,
			valueHelpRequest: [oController.valueHelpHandlerClientes, oController]
		}).addStyleClass("controlFiltro");
		...
		var oFiltroSubCliente = new sap.m.Input(CONSTESTCUENTA.ID.FILTRO_SUBCLIENTE,{
			change: [oController.filtrar, oController],
			showValueHelp: true,
			valueHelpRequest: [oController.valueHelpHandlerSubClientes, oController]
		}).addStyleClass("controlFiltro");
		
		//Por ahora no se muestra, solo guarda el codigo del subcliente
		var oFiltroCodSubCliente = new sap.m.Input(CONSTESTCUENTA.ID.FILTRO_COD_SUBCLIENTE,{
			maxLength:10,
			change: [oController.filtrarCliente, oController]
		}).addStyleClass("controlFiltro");
		////////////////
		...
		var oTable = new sap.ui.table.Table(CONSTESTCUENTA.ID.TABLE,{
			...
			}).addStyleClass("totalTabla"),
			filter: [oController.sumarTotal, oController],
